...
const BookingForm = ({ property }) => {
  ...
  useEffect(() => {
    setNights(Math.round((new Date(checkOut) - new Date(checkIn)) / 1000 / 60 / 60 / 24))    
  }, [checkIn, checkOut])
    ...
    <form className="flex flex-col gap-y-3 px-2 md:px-5 py-2 rounded-md shadow-[0_0_20px_rgba(0,0,0,0.2)] md:w-2/5 md:min-w-80 sticky top-10">
      ...
      <div className="flex flex-col gap-y-3 text-slate-500">
        ...
        <div className="flex justify-between">
          ...
          <div className="">XAF {Math.round(property.price * nights * 1.1)}</div>
        </div>
      </div>
      <button className="text-white bg-primary rounded-md py-2 hover:bg-primary-dull transition-all duration-300">Book Now</button>
    </form>
  );
};

...
export default function UpdateCategoryModal({
        ...
        setCategoryArray((prev) =>
          prev.map((item) => {
            if (item.id === selectedCategory.id) {
              item.name = name.trim() !== "" ? name : selectedCategory.name;
              item.image = image
  ...
  const [isPopular, setIsPopular] = React.useState(selectedCategory.isPopular);
    ...
    >
      ...
      <DialogContent sx={{ maxWidth: "500px" }}>
        <form className="px-10" onSubmit={update}>
          ...
          >
            <img
              src={image ? URL.createObjectURL(image) : selectedCategory.image}
              alt="image"
          ...
          <div className="flex gap-2 items-center my-2">
            <p>Popular:</p>
            <Switch
              defaultChecked={selectedCategory.isPopular}
